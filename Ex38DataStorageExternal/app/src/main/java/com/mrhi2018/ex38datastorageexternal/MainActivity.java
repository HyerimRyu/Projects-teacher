package com.mrhi2018.ex38datastorageexternal;
...
public class MainActivity extends AppCompatActivity {
    ...
    public void clickSave(View view) {
        ...
        if(Build.VERSION.SDK_INT>=19){
            dirs= getExternalFilesDirs("MyDir");//파일이 저장될 폴더명 지정
        }else{
            //19버전 이상의 API기능을 사용하기 위해 호환성버전 클래스(ContextCompat)를 이용.
            dirs= ContextCompat.getExternalFilesDirs(this, "MyDir");
        }

//        Toast.makeText(this, dirs.length+"", Toast.LENGTH_SHORT).show();
        path= dirs[0];
    ...
    //동적퍼미션이 필요한 외부 저장소 경로
    //(본인 앱 패키지명 폴더(storage/../android/data/com.mrhi.xxx..) 말고 나머지 다!!)
    public void clickBtn(View view) {
        ...
        //외부 저장소를 사용하려면 사용자에게 허가(permisson)을 받아야함.
        //Manifest.xml에 퍼미션 사용표시(23버전 미만에서는....이것만 하면 되었음)
        //위 방식은 처음 앱을 다운받을 때 한번만 퍼미션사용을 사용자게에 공지!!
        //위 방식이 보안에 관점에서 문제가 있다는 인식이 생기면서..
        //사용자가 앱을 실행할때.. 마다 마다..물어보게끔..(물론, 한번선택하면 안물어보게 할 수 있음)
        //이런 방식을 동적퍼미션이라고 함!!
        //이제는 이런 방식이 매우 보편화됨..ex. 카메라, 연락처, 위치정보 etc...
    ...
    //requestPermissions()메소드로 보여준 다이얼로그이 선택이
    //완료되면 자동으로 실행된는 메소드
    @Override
