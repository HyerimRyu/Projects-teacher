...
public class MyView extends View {
    ...
    public MyView(Context context) {
        ...
        //행운의 편지 전송(why? 20ms마다 화면갱신을 위해)
        handler.sendEmptyMessageDelayed(0, 20);

    }//constructor

    //이 뷰의 화면을 그려내는 메소드 처음 1번 자동 실행됨.
    @Override
    protected void onDraw(Canvas canvas) {
        //화가객체(Canvas)를 통해서 그려내는 작업코딩
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        //파라미터로 전달된 터치이벤트정보 객체에게 필요한 정보 얻어내기
        ...
        //MotionEvent를 없애기 위해 return true..로..
        return true;//메세지를 소비시키는 역할
    }

    //20ms마다 화면갱신..하기위해 메신저객체 이용
    Handler handler= new Handler(){
        //sendEmptyMessage()를 호출하고 20ms가 지나면 자동실행!!
        @Override
        public void handleMessage(Message msg) {
            invalidate(); //화면갱신( 화면지우고 onDraw()다시 호출)
