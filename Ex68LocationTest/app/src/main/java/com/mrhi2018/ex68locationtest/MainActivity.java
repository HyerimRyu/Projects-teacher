package com.mrhi2018.ex68locationtest;
...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //위치정보제공자
        //1. gps : 가장높은 정확도, 무료, 실내불가, 베터리소모량이 높음
        //2. network : 중간 정확도, 유료or무료, 어디서나 가능, 베터리소모량 중간
        //3. passive : 다른 앱의 마지막 위치정보를 통해서 위치 얻어오기, 정확도가 가징 낮음, 사용빈도 거의 없음
        ...
        //베스트 위치정보제공자 알아내기 (반드시 위치정보사용에 대한 퍼미션 필요)
        ...
        //마시멜로우(api23버전)부터 동적퍼미션 필요 : 앱을 다운로드받을 때 뿐만 아니라 사용할 때도 퍼미션을 체크하는 방식
    ...
    //퍼미션 [허가/거부]여부를 선택하였을때 자동으로 호출되는 메소드
    @Override
    ...
    //위치정보 갱신을 듣는 리스너객체(멤버변수 위치)
    LocationListener locationListener= new LocationListener() {
        @Override
        public void onLocationChanged(Location location) {
            //위치정보 갱신 시마다 Location객체 전달
            double latitude= location.getLatitude();
            double longitude= location.getLongitude();
            ...
            // 왕십리역 좌표 : 37.561239, 127.038369

            //내 위치(latitude, longitude)와 왕십리역사이의 실제거리(m) 계산
            float[] result= new float[3];//거리계산 결과를 저장할 빈 배열 객체
            Location.distanceBetween(latitude, longitude, 37.561239, 127.038369, result);

            //result[0]에 두 좌표사이의 m(미터)거리가 계산되어 저장되어 있음.
